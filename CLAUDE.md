...
- **Download limit:** S3 downloads are capped at 2 GB to prevent OOM from corrupted or malicious objects. The consequence matters more than the guard: because `Open`'s write preload and `ensureLoaded` both go through `Store.Download`, a file above 2 GB **cannot be reopened for writing at all**, and under encryption cannot be read at all once it falls out of cache. The block layout retires this rather than raising the cap — see `claudedocs/block-layout-design.md`.
